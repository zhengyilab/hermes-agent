...
class WebhookAdapter(BasePlatformAdapter):
    ...
    async def _deliver_single(
        ...
        if self.gateway_runner and _is_known_platform:
            # inline_chat_id / inline_thread_id already parsed above from both
            # the deliver_type "platform:chat_id" format and the _inline_* keys
            # set by _deliver_multi. Do NOT re-read delivery here — that would
            # overwrite the parsed target and silently fall back to deliver_extra
            # / home channel.
    ...
    async def _direct_deliver(
        ...
        # "platform:chat_id" parser applies uniformly — _direct_deliver is
        # reached directly by deliver_only routes, which would otherwise
        # forward the raw "platform:chat_id" string to _deliver_cross_platform
        # and fail.
